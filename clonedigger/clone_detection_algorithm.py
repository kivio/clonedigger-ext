...
def findDuplicateCode(source_files, report):
    ...
    def findHugeSequences():
        ...
        return [PairSequences([StatementSequence(s1), StatementSequence(s2)]) for (s1, s2) in
                suffix_tree_instance.getBestMaxSubstrings(arguments.size_threshold, f, f_elem)]
                    ...
                    if (distance < arguments.distance_threshold):
                        ...
                        if first + n < pair_sequences.getLength():
                            pairs_sequences.append(
                                pair_sequences.subSequence(first + n, pair_sequences.getLength() - first - n))
                        n += 1
                        flag = True
                        break
                if flag:
