...
#	    s = '<P> Clone detected in source files "%s" and "%s" <BR>\n' % (sequences[0].getSourceFile().getFileName(), sequences[1].getSourceFile().getFileName())
		...
		if arguments.use_diff:
			...
			u = anti_unification.Unifier(s1, s2, ignore_parametrization=True)
			for i in [0,1]:
			    stmt = (statements[0], statements[1])[i]
			    sub = u.getSubstitutions()[i]
			    for tree in sub.getMap().values():
				n = NewAsString(tree.ast_node.as_string())
				tree.ast_node.as_string = n
			    stmts[i] = sub.substitute(stmt, True) 
			d = [None, None]
			for j in (0,1):
			    d[j] = ''
			    tl = stmts[j].ast_node.as_string().split('\n')
			    for ts in tl:
				indent = re.findall('^ *', ts)[0]
				d[j] += re.sub('^ *', len(indent)/4*'&nbsp; ', ts) + '<BR>\n'

		    except:
			print 'The following error occured during highlighting of differences on the AST level:'
			traceback.print_exc()			
			for j in [0,1]:
			    print clone[j].getSourceFile().getFileName(), '(', min(clone[j][0].getCoveredLineNumbers())+1, ')'
